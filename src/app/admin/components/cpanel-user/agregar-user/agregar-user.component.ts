...
export class AgregarUserComponent implements OnInit {
  ...
  constructor(
    ...
    private _serviceUsuer: UsuarioService,
    private router: Router,
    @Inject(MAT_DIALOG_DATA) public editarDatos: any,
    private dialogRef: MatDialogRef<AgregarUserComponent>
  ) {}
  ...
  agregarUsuario() {
    if (!this.editarDatos && this.actionBtn === 'Guardar') {
      ...
      this.router
        ...
        .then(() =>
          this.router.navigate(['/cpanel/users'], {
            state: { editarDatos: this._serviceUsuer.updateUser(user) },
          })
        );
    }
    this.dialogRef.close();
